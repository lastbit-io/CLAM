...
class Autopilot {
	...
	get_seed_nodes()
	{
		...
		return new Promise((resolve, reject) => {
			dig(['lseed.bitcoinstats.com', 'SRV'])//dig(['lseed.bitcoinstats.com', 'SRV'])test.nodes.lightning.directory soa.nodes.lightning.directory
			.then((result) => {
				for(let i of result.answer)
					node_ids.push(toHexString(bech32.fromWords((bech32.decode(i.value.split('.')[0]).words))));
				console.log("\n\n### Got random seed nodes = ", node_ids + "\n\n");
				resolve(node_ids);
			})
			.catch((err) => {
	...
	{
		shuffle(seed_nodes);
		console.log("\n\n### Connecting to random seed nodes...\n\n");
	...
	populate_graph()
	{
		...
		return new Promise(async (resolve, reject) => {
			...
			await this._lightning.listnodes().then((listnodes) => { //listnodes listpeers
				for(let i of listnodes.nodes) //listnodes.nodes listpeers.peers
					this.G.addNode(i.id, i); //i.nodeid
			}).catch((err) => {
				reject(err);
			});
	...
	calculate_statistics(candidates, wallet_balance)
	{
		return new Promise((resolve, reject) => {
			var pdf = {};
			for(let candidate of candidates)
			{
				...
				if(capacity == 0)
					average = Math.floor(wallet_balance/candidates.length);
	...
	calculate_proposed_channel_capacities(pdf, balance = 1000000)
	{
		return new Promise((resolve, reject) => {
			...
			console.log("pdf_arr", pdf_arr);
			var min_probability = Math.min.apply(null, pdf_arr);
			var needed_total_balance = Math.ceil(minimal_channel_balance/min_probability);
			console.log("needed_total_balance = ", needed_total_balance);
			console.log("### Object.keys(pdf).length= ", Object.keys(pdf).length);
			while(needed_total_balance > balance && Object.keys(pdf).length > 1) 
			{
				...
				console.log("### temp_pdf_arr = ", temp_pdf_arr);
				var min_val = Math.min.apply(null, temp_pdf_arr);
				for(let node in pdf)
				{
					if(pdf[node] == min_val)
						delete pdf[node];
				...
				min_val = Math.min.apply(null, temp_pdf_arr);
				needed_total_balance = Math.ceil(minimal_channel_balance / min_val);
				console.log("### calculate_proposed_channel_capacities pdf = ", pdf);
			}
			resolve(pdf);
		});
	}
	...
	async fund(nodes, balance)
	{
		for(let node in nodes)
		{
			...
			console.log("\n\n###Connecting to node %s to fund channel with amount = %d", node, sats);
	...
	async naive_fund(balance)
	{
		await this._lightning.listpeers().then((listpeers) => { 
			var sats = Math.floor(balance/listpeers.peers.length);
			for(let peer of listpeers.peers)
			{
				console.log("\n\n### Attempting to fund channel with node %s and amount = %d", peer.id, sats);
				this._lightning.fundchannel(peer.id, sats).then((channel) => {
					console.log("Successfully funded channel with suggested node = ", channel);
				}).catch((err) => {
					console.log("!#! Could not fund channel with suggested node = ", err);
				});	
			...
			console.log("!#! Error gettting peers for naive_fund = ", err);
		});	
	}
}
